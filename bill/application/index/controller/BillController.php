...
class BillController extends Controller
{
     ...
     public function index()
    {
        try{
			...
			$bills = $Bill->paginate($pageSize,false,[
			    'query'=>[
 ...
 */   
    public function insert(){
    	...
    	try{
    		...
    		$Total = new Total;
    		//为对象赋值
    		$Bill->total_id = $postData['kind'];
    		$Bill->update_time = date("Y-m-d H:i:s");	
    		$Bill->consume = $postData['consume'];

    		//验证并保存
    		$result = $Bill->validate(true)->save($Bill->getData());
 ...
 */
    public function delete()
    {
        try {
            ...
            // 删除对象
            if (!$Bill->delete()) {
                return $this->error('删除失败:' . $Bill->getError());
            }
 ...
 */    
    public function update()
    {
        ...
        try {
            ...
            if (!is_null($Bill)) {
                // 写入要更新的数据
                $Bill->total_id = input('post.kind');
                $Bill->update_time = date("Y-m-d H:i:s");
                $Bill->consume = input('post.consume');
                

                // 更新
                if (false ===  $Bill->validate(true)->save($Bill->getData())) {
                    return $this->error('更新失败' . $Bill->getError());
                }
            } else {
