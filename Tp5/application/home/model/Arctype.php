...
class Arctype extends Model
{
    ...
    public function archive(){
        return $this->hasMany('Archive','typeid','id');
    }
        ...
        if(!$result){
            ...
            foreach ($result as $k =>$val){
                ...
                if ( $val->arctypeMod->mod == 'addonjump' && !empty($val->jumplink) ){
                    ...
                    $result[$k]['typelink'] = url('@cate/'.$val->dirs);
    ...
    public function position($id, $home, $line){
        ...
        $result = "<a href=\"".url("/home")."\">".$home."</a>".$interval;
        $num = count($positionArr) - 1;
        foreach($positionArr as $k=>$val){
            $result .= '<a href="'.url('cate/'.$val['dirs']).'">'.$val['typename'].'</a>';
            if($k != $num){
                $result .= $interval;
            }
        }
     ...
     * @throws
     */
    public function arctypeCount($pid){
 ...
 */
    ...
    public function page_list($id,$sorts){
        ...
        $result = $this->where($where)->order($sorts)->select();
        foreach ($result as $k => $v) {
            $result[$k]['url'] = url('@cate/'.$v['dirs']);
            $result[$k]['target'] = $v['target'];
            $result[$k]['typename'] = $v['typename'];
        }
        return $result;
    }

    public function arctype_id($sorts){
        if (isAdmin()) {
            $where = [
             'status'=> 1,
             'pid' => 1
        ...
        return $arctype = $this->where($where)->order($sotrs)->select();
    }
