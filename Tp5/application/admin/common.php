 ...
 * @return string
 * @author 苏晓信
 * @date 2017年8月21日(2018-2-19)
 * @edit 不需要request()->path()得到当前参数，地址栏会自动填充，也不需要$path参数
 * @throws
 */
function table_sort($param,$path){
    $url_path = $path;
    $faStr = 'fa-sort';//class属性
    $get = input('get.');
    if( isset($get['_pjax']) ){ unset($get['_pjax']); }//？
    ...
    if( isset($get['_sort']) ){   //判断是否存在排序字段
        ...
        $get['_sort'] = $param.',asc';//ascending order
    }
    $paramStr = [];
    foreach ($get as $k=>$v){
        $paramStr[] = $k.'='.$v;
 ...
 */
    function thumb($param){
        $url_path = 'detail/thumb';
        $aid = $param;
        $uid = session('webuserId');
        $params = $aid.','.$uid;
        $result = $url_path.'?'.$params;
        return $result;
    }
/**
 * @Title: unset_array
