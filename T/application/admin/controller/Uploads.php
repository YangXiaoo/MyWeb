...
class Uploads extends Controller
{
    ...
    public function initialize()
    {
        ...
        $this->file_move_path = WEB_PATH.DIRECTORY_SEPARATOR.$up['upload_path'].DIRECTORY_SEPARATOR.$this->up_type;
        $this->file_back_path = DIRECTORY_SEPARATOR.$up['upload_path'].DIRECTORY_SEPARATOR.$this->up_type;
    ...
    {
        $up_config = cache('DB_UP_CONFIG');
        $file = request()->file('imgFile');
        if ($file){
            $info = $file->validate([
                'size' => $up_config['upload_size'] * 1048576,              //验证文件大小
                'ext' => $up_config[$this->up_type.'_format']   //验证文件格式
            ])->move($this->file_move_path);
            
            if($info){
                if( $this->up_type == 'image' && $up_config['isprint'] == 1 ){   //上传图片，加水印
                    $file = Image::open( $this->file_move_path.DIRECTORY_SEPARATOR.$info->getSaveName() );   //打开上传的图片
                    $file->water(
                        WEB_PATH.$up_config['print_image'],     //水印图片
                        $up_config['print_position'],           //水印位置
                        $up_config['print_blur']                //水印透明度
                    )->save( $this->file_move_path.DIRECTORY_SEPARATOR.$info->getSaveName() );      //保存同名图片并覆盖替换
                }
                
                $file_path = $this->file_back_path.DIRECTORY_SEPARATOR.$info->getSaveName();
                $file_path = $up_config['file_url'].str_replace('\\', '/', $file_path);
                echo json_encode(['error' => 0, 'url' => $file_path, 'info' => lang('upload_success')]);
            }else{
                $this->_alert($file->getError());
            }
        }else{
            $this->_alert(lang('upload_choose'));
        }
    }
    ...
    public function cropper()
    {
        ...
        if ($file){
            ...
            if ( !file_exists($this->file_move_path.DIRECTORY_SEPARATOR.date('Ymd', time())) ){
                ...
                mkdir($this->file_move_path.DIRECTORY_SEPARATOR.date('Ymd', time()));
            }
            ...
            if (file_exists($path)){    //检测图片是否保存成功
                ...
                if ($result){   //保存成功再删除旧头像
                    if ($oldAvatar != config('default_avatar')){      //删除之前头像
    ...
    public function _alert($msg)
    {
        ...
        echo json_encode(['error' => 1, 'message' => $msg, 'info' => $msg]);
        exit;
    }
}
